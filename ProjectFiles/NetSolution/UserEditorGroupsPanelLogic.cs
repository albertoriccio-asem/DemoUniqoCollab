...
public class UserEditorGroupsPanelLogic : QPlatform.NetLogic.BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            if (editable.Value)
            {
                var groupCheckBox = InformationModel.MakeObject<Panel>(group.BrowseName, DemoUniqoCollab.ObjectTypes.GroupCheckbox);
            ...
            {
                var groupLabel = InformationModel.MakeObject<Panel>(group.BrowseName, DemoUniqoCollab.ObjectTypes.GroupLabel);
